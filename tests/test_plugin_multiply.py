'''This file contains the tests for the 'multiply' command in the app'''
import pytest
from app import App
...
def test_app_multiply_variations(capfd, monkeypatch, input1, input2, expected_output):
    ...
    inputs = iter(['multiply', input1, input2, 'exit'])
    monkeypatch.setattr('builtins.input', lambda _: next(inputs))
...
def test_app_multiply_non_numeric(capfd, monkeypatch):
    ...
    inputs = iter(['multiply', 'notANumber', '5', 'exit'])
    monkeypatch.setattr('builtins.input', lambda _: next(inputs))
