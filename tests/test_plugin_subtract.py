...
def test_app_subtract_variations(capfd, monkeypatch, minuend, subtrahend, expected_output):
    """Test the 'subtract' command with various inputs."""
    inputs = iter(['subtract', minuend, subtrahend, 'exit'])
    monkeypatch.setattr('builtins.input', lambda _: next(inputs))
...
def test_app_subtract_non_numeric(capfd, monkeypatch):
    """Test the 'subtract' command with non-numeric input."""
    inputs = iter(['subtract', 'ten', 'three', 'exit'])
    monkeypatch.setattr('builtins.input', lambda _: next(inputs))
